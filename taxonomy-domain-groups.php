...
add_filter( 'body_class', 'taxonomy_ski_domains_body_class' );
/**
 ...
 * @return array
 */
function taxonomy_ski_domains_body_class( $classes ) {

	$classes[] = 'ski-domains-archive';

	return $classes;

}

add_filter( 'genesis_attr_entry-content', 'taxonomy_ski_domains_class' );
/**
 ...
 * @return mixed
 */
function taxonomy_ski_domains_class( $classes ) {
...
add_filter( 'genesis_attr_archive-pagination', 'archive_ski_domains_pagination_attr' );
/**
 * Afegim una css id "ski-domains-archive-pagination" a entry-content per al CPT ski-resort.
 *
 * @since   1.0.0
 *
 * @param $attributes
 ...
 * @return mixed
 */
function archive_ski_domains_pagination_attr( $attributes ) {

	if ( is_post_type_archive( 'ski-domain' ) || is_tax( 'locations' ) ) {
		$attributes['id'] = 'ski-domains-archive-pagination';
	}
...
add_action( 'genesis_before_while', 'taxonomy_domain_groups_wrap_start' );
function taxonomy_domain_groups_wrap_start() {

	?>
	<div class="flexbox-taxonomy-domain-groups">

	<div class="taxonomy-domain-groups-aside-wrap">

		<div class="taxonomy-domain-groups-aside-content">
			<?php

			$domain_groups_prepend          = get_term_meta( get_queried_object_id(), 'domain_groups_breadcrumbs_prepend', true );
			$fc_domain_groups_breadcrumbs   = get_term_meta( get_queried_object_id(), 'fc_domain_groups_breadcrumbs', true );

			foreach ( ( array ) $fc_domain_groups_breadcrumbs as $count_fc_dgb => $fc_domain_groups_breadcrumb ) {

				switch ( $fc_domain_groups_breadcrumb ) {

					case 'layout_domain_groups_breadcrumbs':

						$domain_groups_breadcrumb_type              = get_term_meta( get_queried_object_id(), 'fc_domain_groups_breadcrumbs_' . $count_fc_dgb .  '_domain_groups_breadcrumb_type', true );
						$count_repeater_domain_groups_breadcrumbs   = get_term_meta( get_queried_object_id(), 'fc_domain_groups_breadcrumbs_' . $count_fc_dgb .  '_repeater_domain_groups_breadcrumbs', true );

						if ( $count_repeater_domain_groups_breadcrumbs ) {

							echo '<p id="domain-groups-type"><strong>' . $domain_groups_breadcrumb_type . '</strong></p>';

							for ( $i = 0; $i < $count_repeater_domain_groups_breadcrumbs; $i++ ) {

								$breadcrumb = get_term_meta( get_queried_object_id(), 'fc_domain_groups_breadcrumbs_' . $count_fc_dgb .  '_repeater_domain_groups_breadcrumbs_' . $i . '_domain_group_breadcrumb', true );
								$url        = get_term_meta( get_queried_object_id(), 'fc_domain_groups_breadcrumbs_' . $count_fc_dgb .  '_repeater_domain_groups_breadcrumbs_' . $i . '_domain_group_url', true );

								echo '<p id="domain-groups-breadcrumbs"><a href="' . $domain_groups_prepend . $url . '" rel="bookmark">' . $breadcrumb . '</a></p>';
	...
	<div class="taxonomy-domain-groups-content-wrap">
	<?php

}
add_action( 'genesis_after_loop', 'taxonomy_domain_groups_wrap_end' );
function taxonomy_domain_groups_wrap_end() {
