...
//include $script;
//include ( $_SERVER['DOCUMENT_ROOT'] . "/wp-content/themes/cameraski/lib/meteo/yrnosettings.php" ); // CARLES LOCAL
// AFEGIT
//echo '<!-- trying to load ' . $script . ' -->' . PHP_EOL;

//$script	= $scriptDir . 'yrnoCreateArr.php'; // CARLES LOCAL

//include $script;
//include ( $_SERVER['DOCUMENT_ROOT'] . "/wp-content/themes/cameraski/lib/meteo/yrnoCreateArr.php" ); // CARLES LOCAL
// AFEGIT
//echo '<!-- trying to load ' . $script . ' -->' . PHP_EOL;


$script		= site_url( '/wp-content/themes/cameraski/lib/meteo/yrnosettings.php' ); // CARLES SERVER

include ( $_SERVER['DOCUMENT_ROOT'] . "/wp-content/themes/cameraski/lib/meteo/yrnosettings.php" ); // CARLES SERVER
// AFEGIT
//echo '<!-- trying to load ' . $script . ' -->' . PHP_EOL;

$script		= $scriptDir . 'yrnoCreateArr.php'; // CARLES SERVER

include ( $_SERVER['DOCUMENT_ROOT'] . "/wp-content/themes/cameraski/lib/meteo/yrnoCreateArr.php" ); // CARLES SERVER
// AFEGIT
//echo '<!-- trying to load ' . $script . ' -->' . PHP_EOL;
...
for ( $i = 0; $i < count( $returnArray['forecast'] ); $i++ ) {
	...
	// $nightDayBefore		= true; // if night not listed for previous day -> CARLES 2017-03-02 -> Això estava sense comentar.
  	/* METEO-WEATHER */
...
$tableIcons_1	= '
<table class="overviewTable centerTable" style="background-color: transparent;">
...
$iconWidth	= 100 / $topCount;
$tableIconsMobile_1	= '
<table class="mobileOverviewTable mobileCenterTable" style="background-color: transparent;">
	<tbody>
...
/*
//$script = $scriptDir . '/wp-content/themes/cameraski/lib/meteo/yrnoCreateDetailArr.php';
$script = $scriptDir . 'yrnoCreateDetailArr.php';
echo '<!-- trying to load ' . $script . ' -->' . PHP_EOL;
//include $script;
/* METEO-WEATHER */
/*include ( $_SERVER['DOCUMENT_ROOT'] . "/wp-content/themes/cameraski/lib/meteo/yrnoCreateDetailArr.php" );
/* / METEO-WEATHER */
...
function myDateLinePrint( $time ) {
	...
	$string			='<tr class="overviewYrnodateline ' . $rowColor . '">
						<td colspan="' . $cols . '">
